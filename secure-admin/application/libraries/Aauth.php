...
//        $this->CI->load->helper('googleauthenticator_helper');
    ...
    public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
        ...
        $query = $this->aauth_db->where('password', md5(trim($pass)));
        $query = $this->aauth_db->where('status', 1);
        $query = $this->aauth_db->where('is_deleted', 0);
        ...
        if ($query->num_rows() != 0) {
            ...
            $priviledgeData = $this->aauth_db->select('group_privilege')->where('group_id', $data['admin_group_id'])->get('user_group')->row_array();
            $this->CI->session->set_userdata($data);
//            $this->CI->session->set_userdata('admin_priviledge', $priviledgeData['group_privilege']);
            $_SESSION['admin_priviledge'] = $priviledgeData['group_privilege'];
            return TRUE;
        }
        // if not matches
        else {
    ...
    public function is_loggedin() {
        ...
        if ($this->CI->session->userdata('admin_loggedin')) {
            $query = $this->aauth_db->where('user_id', $this->CI->session->userdata('admin_id'));
            $query = $this->aauth_db->where('status', 1);
            $query = $this->aauth_db->where('is_deleted', 0);
            $query = $this->aauth_db->get($this->config_vars['users']);
            $row = $query->row();
...
//            $message = $this->CI->lang->line('aauth_email_reset_text');
...
//            $this->CI->email->send();
            $email_html = '<table cellpadding="4" cellspacing="1">
	<tbody>
		<tr>
			<td colspan="2">Dear ' . $first_name . ' ' . $last_name . ',</td>
		</tr>
		<tr>
			<td colspan="2">Your Password has been reset on '.SITE_TITLE.' and its detail are as follows</td>
		</tr>
		<tr>
			<td colspan="2">&nbsp;</td>
		</tr>
		...
		<tr>
			<td colspan="2"><strong>Click On This Link to get Login in to your profile : <a href="' . base_url() . '">' . base_url() . '</a></strong></td>
		</tr>
	</tbody>
</table>
';
            sendMail($email, 'New Password', $email_html, '', '', array());
            return TRUE;
        }
        return FALSE;
    }
...
//        }
        ...
        if ($this->user_exist_by_email($data['email'])) {
...
//            'pass' => $this->hash_password($pass, 0), // Password cannot be blank but user_id required for salt, setting bad password for now
            'password' => md5($data['password']), // Password cannot be blank but user_id required for salt, setting bad password for now
...
//            $this->aauth_db->update($this->config_vars['users'], $data);
     ...
     * @return bool Update fails/succeeds
     */
    public function update_user($user_id, $data, $email = FALSE, $pass = FALSE, $name = FALSE) {
...
//            'pass' => $this->hash_password($pass, 0), // Password cannot be blank but user_id required for salt, setting bad password for now
        ...
        if(!empty($data['image'])){
            $upddata['image'] = $data['image'];
        }
        if (!empty($data['password']))
            $upddata['password'] = md5($data['password']);
        if (!$valid || empty($data)) {
            return FALSE;
        }
        $this->aauth_db->where('user_id', $user_id);
        return $this->aauth_db->update($this->config_vars['users'], $upddata);
    }
     ...
     */
    public function list_users($notId = 0, $group_id = 2, $group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE) {
...
//                    ->join($this->config_vars['user_to_group'], $this->config_vars['users'] . ".id = " . $this->config_vars['user_to_group'] . ".user_id")
            ...
            if ($notId) {
                $login_id = $this->CI->session->userdata('admin_id');
//                $query = "select a.*,b.group_name from " . $this->config_vars['users'] . " a left join " . $this->config_vars['groups'] . " b on a.group_id=b.group_id where a.user_id != " . $notId . " and a.user_id != " . $login_id . " and a.is_deleted=0";
                $query = "select a.*,b.group_name from " . $this->config_vars['users'] . " a left join " . $this->config_vars['groups'] . " b on a.group_id=b.group_id where a.user_id != " . $notId . " and a.user_id != " . $login_id . " and a.is_deleted=0";
                $query = $this->aauth_db->query($query);
            } else {
                $this->aauth_db->select('*')
                    ->from($this->config_vars['users']);
            }
...
//        $this->aauth_db->delete($this->config_vars['user_variables']);
        // delete user
        $this->aauth_db->where('user_id', $user_id);
        return $this->aauth_db->update($this->config_vars['users'], array('is_deleted' => 1));
//        return $this->aauth_db->delete($this->config_vars['users']);
    ...
    public function create_group($data, $group_name = '', $definition = '') {
...
//        $this->info($this->CI->lang->line('aauth_info_group_exists'));
     ...
     */
    public function update_group($group_id, $data, $group_par = '', $group_name = FALSE, $definition = FALSE) {
...
//        $this->aauth_db->delete($this->config_vars['perm_to_group']);
        ...
        $this->aauth_db->update($this->config_vars['users'], array('is_deleted' => 1));
        $this->aauth_db->where('group_id', $group_par);
        $result = $this->aauth_db->update($this->config_vars['groups'], array('is_deleted' => 1));
        return $result;
//        $this->aauth_db->delete($this->config_vars['groups']);
     ...
     */
    public function list_groups($is_condition = false, $condition = array()) {
        if ($is_condition)
            $this->aauth_db->where($condition);
        $query = $this->aauth_db->get($this->config_vars['groups']);
        return $query->result_array();
    }
    ...
    public function get_group($group_id, $condition) {

        $query = $this->aauth_db->where('group_id', $group_id);
        $query = $this->aauth_db->where($condition);
        $query = $this->aauth_db->get($this->config_vars['groups']);
